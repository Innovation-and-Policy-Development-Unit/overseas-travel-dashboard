...
import { MinistryBreakdown } from './pages/MinistryBreakdown';
import { TravelTracker } from './pages/TravelTracker';
import { loadTravelRows } from './lib/data';
...
function App() {
  ...
  useEffect(() => {
    ...
    loadTravelRows()
      ...
      .catch(err => {
        if (err.name !== 'AbortError') { setError(String(err.message)); setLoading(false); }
      });
  ...
  return (
    <div className="flex min-h-screen bg-un-canvas">
      ...
      {/* Sidebar */}
      <aside
        className={`${isDesktop ? 'sticky top-0 h-screen flex-shrink-0' : `fixed inset-y-0 left-0 z-30 transition-transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}`}
        ...
        {!isDesktop && (
          <header className="flex items-center gap-3 border-b border-un-border bg-un-surface px-4 py-3">
            <button
              ...
              className="rounded-md p-1.5 text-un-secondary hover:bg-un-wash"
            >
        ...
        <main className="flex-1 overflow-auto p-6" aria-live="polite">
          {loading ? (
            ...
            <Routes>
              ...
              <Route path="/executive" element={<ExecutiveOverview rows={rows} />} />
              <Route path="/ministry" element={<MinistryBreakdown rows={rows} />} />
              <Route path="/tracker" element={<TravelTracker rows={rows} />} />
            </Routes>
          )}
        </main>
      </div>
